...
let orthoZoom;
let oldMat;
export const ControlledCameras = /*#__PURE__*/ forwardRef(
  (
    {
      mode,
    ...
    const [persCam, orthoCam] = useMemo(() => {
      ...
      set(() => ({
        camera: mode === "perspective" ? persCam : orthoCam,
      }));
    ...
    const controls = useMemo(() => {
      ...
      if (!persPos) persPos = perspectivePosition.slice();
      if (!persTarget) persTarget = perspectiveTarget.slice();
      if (!orthoPos) orthoPos = orthographicPosition.slice();
      if (!orthoTarget) orthoTarget = orthographicTarget.slice();
      return controls;
    ...
    useEffect(() => {
      ...
      persPos = perspectivePosition.slice();
    ...
    useEffect(() => {
      ...
      persTarget = perspectiveTarget.slice();
    ...
    useEffect(() => {
      ...
      orthoPos = orthographicPosition.slice();
    ...
    useEffect(() => {
      ...
      orthoTarget = orthographicTarget.slice();
    ...
    useEffect(() => {
      (async (_animation$current) => {
        controls.enabled = false; // Disable input during transition
        const currentMode = controls.camera.isPerspectiveCamera
          ? "perspective"
          : "orthographic";
        ...
        (_animation$current = animation.current) === null ||
          _animation$current === void 0 ||
